...
p1 = np.array([147,292,241])
p_org = np.array([150,292,244])
p6 = np.array([151 ,292,246])
...
sitk.WriteImage(v_above,r"C:\My_Data\BRC_Project\codes\march5/"+ "v1_above.nii.gz")
...
sitk.WriteImage(v_below,r"C:\My_Data\BRC_Project\codes\march5/"+ "v1_below.nii.gz")
...
v_below = sitk.GetImageFromArray(v_below)
v_below.CopyInformation(img_itk)
sitk.WriteImage(v_below,r"C:\My_Data\BRC_Project\codes\march5/"+ "The plane.nii.gz")


volume = np.zeros(image_data.shape)
volume_shape = (180,666,512)

for x in range(volume_shape[0]):
    for y in range(volume_shape[1]):
        for z in range(volume_shape[2]):
            # Calculate the signed distance of the voxel from the plane
            signed_distance = signed_distance_from_plane((x, y, z), plane_equation)
            
            # Check if voxel lies on the plane, above, or below
            if signed_distance == 0:
                volume[x, y, z] = 1  # Voxel lying on the plan



v_below = sitk.GetImageFromArray(volume)
v_below.CopyInformation(img_itk)
sitk.WriteImage(v_below,r"C:\My_Data\BRC_Project\codes\march5/"+ "The plane3.nii.gz")
