...
def cal_SJ(img_paths):
    ...
    for i in range(10):
        ...
        #image1 = Normalization_2(image1)
        hist1, bin_edges  = np.histogram(image1, bins=bins, density=True)

        for j in range(10):  
            if j!=i:
                image2 =  sitk.GetArrayFromImage(sitk.ReadImage(img_paths[j]))
                #image2 = Normalization_2(image2)
                hist2, _ = np.histogram(image2, bins=bin_edges,  density=True)
    ...
    return np.mean(summ_all),image1
...
if __name__ == "__main__":
    ...
    avg_js_dist_1,image1  = cal_SJ(img_paths)
